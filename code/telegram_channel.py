...
class Session:
    def __init__(self):
        self.__sender_id = 502136423

    # id paramater is int
    def set_sender_id(self, id):
        self.__sender_id = id
...
class TelegramOutput(Bot, OutputChannel):
    ...
    def send_text_message(self, recipient_id, message):
        return self.send_message(chat_id = recipient_id, text = message, parse_mode = ParseMode.MARKDOWN)

    def send_image_url(self, recipient_id, image_url):
        return self.send_photo(recipient_id, image_url)

    def send_text_with_buttons(self, recipient_id, text,
                               buttons, button_type="vertical", **kwargs):
    ...
    def blueprint(self, on_new_message):
        ...
        def message():
            if request.method == 'POST':
                ...
                session.set_sender_id(sender_id)
                print("ID: " + str(sender_id))
                try:
